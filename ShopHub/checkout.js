/* =========================================================
   ShopHub Marketplace — checkout.js
...
function renderSummary(cart) {
  document.getElementById("checkoutCurrency").textContent = window.SHOPHUB_CONFIG.symbol;
  summaryItems.innerHTML = cart
    .map((item) => `<div>${item.qty} x ${escapeHtml(item.name)} — ${window.SHOPHUB_CONFIG.symbol} ${(item.qty * item.price).toFixed(2)}</div>`)
    .join("");
  checkoutTotal.textContent = ShopHubCartTotal().toFixed(2);
}
...
/** Inserts the order row into shophub_orders. Returns the created order, or null on failure. */
...
document.getElementById("paystackBtn").addEventListener("click", async () => {
  ...
  const handler = PaystackPop.setup({
    key: window.SHOPHUB_PAYSTACK_PUBLIC_KEY,
    email: currentUser.email,
    amount: Math.round(total * 100), // GHS to pesewas
    currency: window.SHOPHUB_CONFIG.paystack_currency,
...
document.getElementById("whatsappBtn").addEventListener("click", async () => {
  ...
  const cart = ShopHubGetCart();
  const lines = cart.map((item) => `• ${item.qty} x ${item.name} — ${window.SHOPHUB_CONFIG.symbol} ${(item.qty * item.price).toFixed(2)}`);
  const message =
    `Hi ShopHub Marketplace! I'd like to place an order:\n\n` +
    lines.join("\n") +
    `\n\nTotal: ${window.SHOPHUB_CONFIG.symbol} ${ShopHubCartTotal().toFixed(2)}` +
    `\n\nName: ${details.name}` +
    `\nPhone: ${details.phone}` +
  ...
  const waUrl = `https://wa.me/${window.SHOPHUB_WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`;
