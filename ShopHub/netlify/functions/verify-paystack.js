/* =========================================================
   ShopHub Marketplace — netlify/functions/verify-paystack.js
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const orderRes = await fetch(
      `${SUPABASE_URL}/rest/v1/shophub_orders?id=eq.${encodeURIComponent(reference)}&select=id,user_id,total,payment_method,payment_status`,
      {
        headers: {
    ...
    const updateRes = await fetch(`${SUPABASE_URL}/rest/v1/shophub_orders?id=eq.${encodeURIComponent(reference)}`, {
      method: "PATCH",
      headers: {
        apikey: SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
