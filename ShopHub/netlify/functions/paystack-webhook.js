/* =========================================================
   ShopHub Marketplace — netlify/functions/paystack-webhook.js
...
exports.handler = async (event) => {
  ...
  if (payload.event === "charge.success") {
    const reference = payload.data.reference; // this is the shophub_orders.id, see checkout.js
    const amountPaid = payload.data.amount / 100;

    try {
      const orderRes = await fetch(
        `${SUPABASE_URL}/rest/v1/shophub_orders?id=eq.${encodeURIComponent(reference)}&select=id,total,payment_status`,
        {
          headers: {
      ...
      if (order && order.payment_status !== "paid" && Math.abs(amountPaid - Number(order.total)) < 0.01) {
        await fetch(`${SUPABASE_URL}/rest/v1/shophub_orders?id=eq.${encodeURIComponent(reference)}`, {
          method: "PATCH",
          headers: {
            apikey: SUPABASE_SERVICE_ROLE_KEY,
            Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
