/* =========================================================
   ShopHub Marketplace — admin.js
   Runs on admin.html:
     - Email/password login (Supabase Auth), restricted to
       window.SHOPHUB_ADMIN_EMAIL (real enforcement is via RLS,
       see setup.sql — this check just improves the UI).
     - Add / edit / delete products in shophub_products.
     - Orders tab: list every order in shophub_orders and update
       its order_status.
   ========================================================= */
  ...
  try {
    ...
    if (editingProductId.value) {
      const { error: updateError } = await supabaseClient
        .from("shophub_products")
        .update(payload)
        .eq("id", editingProductId.value);
      if (updateError) throw updateError;
      showMsg(productMsg, "Product updated successfully.", "success");
    } else {
      const { error: insertError } = await supabaseClient.from("shophub_products").insert([payload]);
...
function productRowHtml(product) {
  return `
    <tr data-id="${product.id}">
      ...
      <td>${window.SHOPHUB_CONFIG.symbol} ${Number(product.price).toFixed(2)}</td>
      <td>${product.stock}</td>
      <td>
...
function orderRowHtml(order) {
  ...
  return `
    <tr data-id="${order.id}">
      ...
      <td>${window.SHOPHUB_CONFIG.symbol} ${Number(order.total).toFixed(2)}</td>
